...
## 十二、设计规则（Design Tokens，Day 9 追加）

> 这一节把"好看"变成可执行的规则。改页面时先查这一节，不要凭感觉定颜色、字号和间距。

### 1. 颜色

| 用途 | 变量 / 色值 | 说明 |
|---|---|---|
| 页面底 | `--bg-page #f6f6f4` | 浅米灰 |
| 面板 / 卡片 | `--bg-panel #ffffff`、`--bg-soft #fbfbf9` | 白与近白两档 |
| 正文 | `--text-main #2c2c2a` | 最高层级 |
| 次要文字 | `--text-soft #5f5e5a` | 说明、标签（6.49:1） |
| 提示小字 | `--text-hint #6b6a65` | 13px 提示语用；**Day 9 由 #888780 加深**，四种底色下均 ≥4.5:1 |
| 强调色 | `--accent #185fa5` | 唯一主色，用于主按钮与关键数字 |
| 结论色 | 绿 `#0f6e56`（已修够）/ 红 `#a32d2d`（还差、不及格） | **只用于结论**，不做装饰 |

**对比度规则**：小字（<18px）对比度必须 **≥4.5:1**。新增配色前先算，再写进样式。
...
|---|---|---|
| 页面标题 | 22px / 600 | 顶部 h1 |
| 板块标题 | 18px / 600 | 四个板块的 h2 |
| 卡片标题 | 15px / 600 | 板块名、课程名 |
| 正文 | 15px | 默认 |
| 说明 / 次要 | 13px | 提示语、卡片小字 |
| 数据指标 | 22px / 600 | 绩点板块的数字 |
| 计数说明 | 16px / 500 | "3 个板块 / 3 门课程"这类**不是指标**的文字，不得用 22px |
...
4 · 8 · 12 · 16 · 24 · 32  (px)
```
**Day 9 已从 13 种乱值收敛到这 6 档**。新增样式时只能从中取值；需要更大的间隔就跳到下一档，不要新造数值。

### 4. 圆角与边框

- 面板 12px、卡片与按钮 8px、徽标 999px（胶囊）
- 边框统一 `1px` 实线（`--border #dcdcd6`）；**不使用阴影、渐变、发光**
...
|---|---|
| 默认 | 见上面的颜色表 |
| hover | 底色加深一档（0.15s 过渡） |
| active | 再深一档；若底色变深导致文字看不清，**文字也要一起加深**（如危险按钮按下时文字 `#791f1f`） |
| focus-visible | 2px 主色焦点框，偏移 2px（键盘可达性） |
| disabled | 灰底灰字 + `cursor: not-allowed`；**能改才显示可点，不能改就直接禁用**，不要"点了才提示" |

### 6. 布局与响应式

- 断点：**≤720px 视为手机宽度**
- 桌面：四个板块 **2×2**；手机：**单栏纵向堆叠**
- **同一行的板块必须等高**（`align-items: stretch`）——不能只按内容高度，否则底边不齐，数据变多时差距更大
- 手机端：数字区改单列；按钮与输入框最小高度 **44px**；任何情况下**不得出现横向滚动条**
- 容器一律用 `minmax(0, 1fr)` 防止内容撑破栅格

### 7. 明确禁止

❌ 渐变背景 ❌ 阴影/发光 ❌ 与产品无关的装饰图形 ❌ 第二种强调色 ❌ 用 22px 大字号显示说明文字
